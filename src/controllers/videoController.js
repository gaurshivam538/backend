...
//   const cachedData = JSON.parse(data); hama upstash mai parse karna ke jarurat nahi hoti upstash parse karka deta hai
...
const deleteVideo = asyncHandler(async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, {}, true, "Video is successfully deleted"));
  } catch (error) {
...
const updateVideo = asyncHandler(async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, video, true, "Video successfully updated"));
  } catch (error) {
    console.error(error);
    throw new ApiError(
      500,
      error.message || "Video details could not be updated"
    );
  }
});
    ...
    return res
      ...
      .json(
        new ApiResponse(
          ...
          AfterIsPublished,
          true,
          "IsPublished is Successfully toggele"
        )
      );
  } catch (error) {
...
const getVideoBySubscribedChannel = async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, videos, "Video is sucessfully find"));
};
